...
SAMDInfrared::SAMDInfrared(Pin& pinOut, Pin& pinIn, LowLevelTimer& timer, int16_t id)
{
    this->id = id;
    recvState = PULSE_RECV_ERROR;
    sending = false;
    outBuffer = NULL;
    pinOut.setDigitalValue(0);
}

void SAMDInfrared::setupGapEvents() {
    devMessageBus.listen(inpin->id, DEVICE_PIN_EVT_PULSE_HI, this, &SAMDInfrared::pulseGap,
                         MESSAGE_BUS_LISTENER_IMMEDIATE);
    devMessageBus.listen(inpin->id, DEVICE_PIN_EVT_PULSE_LO, this, &SAMDInfrared::pulseMark,
                         MESSAGE_BUS_LISTENER_IMMEDIATE);
    listen();
}

void SAMDInfrared::listen() {
    pinIn.getDigitalValue();
    pinIn.eventOn(DEVICE_PIN_EVENT_ON_PULSE);
}

void SAMDInfrared::setupPWM() {
    pinOut.setAnalogPeriodUs(1000 / 38); // 38kHz
    pinOut.setAnalogValue(333);
    pwmstate = enabled;
}

void SAMDInfrared::finishPWM() {
    pin->setAnalogValue(0);
    setPWM(1);
}

void SAMDInfrared::send(Buffer d) {
    if (sending)
        return; // error code?
    ...
    setPeriodicCallback(PULSE_PULSE_LEN, this, (void (*)(void *)) & SAMDInfrared::process);
    while (sending) {
        fiber_sleep(10);
    }
    fiber_sleep(5);
...
void SAMDInfrared::finish(int code) {
...
void SAMDInfrared::addPulse(int v) {
...
void SAMDInfrared::pulseGap(Event ev) {
    if (sending)
        return;
...
void SAMDInfrared::packetEnd(Event)
{
    if (pulsePtr < 5)
        return;
...
void SAMDInfrared::pulseMark(Event ev) {
    if (sending)
        return;
    ...
    lastMarkTime = timer.captureCounter();
...
bool SAMDInfrared::isReciving() {
    auto now = timer.captureCounter();
    // inpin low means mark
    if (pinIn.getDigitalValue() == 0 || now - lastMarkTime < 10000) {
        return true;
    }
    return false;
}

Buffer SAMDInfrared::getBuffer() {
    incrRC(outBuffer);
    return outBuffer;
}

void SAMDInfrared::process() {
    if (!sending)
        return;
